    ...
    def get_hidden_dim(self, module_name: str, layer_idx: int) -> Tuple[int, int]:
        if module_name == "qkv_proj":
            ...
            if isinstance(layer, Qwen3_5AttentionDecoderLayer):
                q_output_dim = layer.q_size * (1 + int(layer.attn_output_gate))
                kv_output_dim = layer.kv_size
                return self.hidden_size, q_output_dim + kv_output_dim + kv_output_dim
            if isinstance(layer, Qwen3_5LinearDecoderLayer):
                linear_attn = layer.linear_attn
                return self.hidden_size, linear_attn.key_dim * 2 + linear_attn.value_dim
            raise NotImplementedError(
                f"get_hidden_dim for {module_name} is not defined on {type(layer).__name__}"
            )

        if module_name == "o_proj":
            layer = self.layers[layer_idx]
            if isinstance(layer, Qwen3_5AttentionDecoderLayer):
                return layer.total_num_heads * layer.head_dim, self.hidden_size
            if isinstance(layer, Qwen3_5LinearDecoderLayer):
                return layer.linear_attn.value_dim, self.hidden_size
            raise NotImplementedError(
                f"get_hidden_dim for {module_name} is not defined on {type(layer).__name__}"
            )

        if module_name in {"gate_proj", "up_proj"}:
            return self.hidden_size, self.config.intermediate_size
        if module_name == "gate_up_proj":
            return self.hidden_size, self.config.intermediate_size * 2
        if module_name == "down_proj":
            return self.config.intermediate_size, self.hidden_size
        if module_name == "embed_tokens":
            return self.config.vocab_size, self.hidden_size
        if module_name == "lm_head":
            return self.hidden_size, self.config.vocab_size
